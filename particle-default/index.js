...
const PARTICLE_LIMIT = 2;
const PARTICLE_ITER_LENGTH = 1;
  ...
  while (1) {
    if (particleSet.size < PARTICLE_LIMIT) {
      ...
      Array.from({length}).forEach(() => {
        ...
        const grow = () => {
          dot.grow();
          ctx.fillRect(canvas.width * dot.x, canvas.height * 0.5, PARTICLE_SIZE, PARTICLE_SIZE);
          dotx.innerText = 'x: ' + dot.x.toFixed(4);
          dotvx.innerText = 'velocity: ' + dot.velX;
          dotax.innerText = 'accelation: ' + dot.accX;
          dotleft.innerText = 'left: ' + (canvas.width * dot.x).toFixed(4);
          if (dot.x > 1 
            // || dot.y > 1 
            || dot.x < 0 
            // || dot.y < 0
            ) particleSet.delete(grow);
        };
